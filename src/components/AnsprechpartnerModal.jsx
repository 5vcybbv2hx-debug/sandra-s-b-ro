...
export default function AnsprechpartnerModal({ open, onClose, onCreated, editPerson, presetFirmaId }) {
  const [form, setForm] = useState({ vorname: '', nachname: '', firma_id: '', rolle: 'Sonstiges', telefon: '', email: '', notizen: '' });
  ...
  useEffect(() => {
    if (open) {
      ...
      setForm(editPerson || { vorname: '', nachname: '', firma_id: presetFirmaId || '', rolle: 'Sonstiges', telefon: '', email: '', notizen: '' });
    }
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><Label>Firma</Label><Select value={form.firma_id || '_none'} onValueChange={(v) => setForm({ ...form, firma_id: v === '_none' ? '' : v })}><SelectTrigger className="min-h-[48px]"><SelectValue placeholder="Keine Firma (Privatperson)" /></SelectTrigger><SelectContent><SelectItem value="_none">Keine Firma (Privatperson)</SelectItem>{firmen.map((f) => <SelectItem key={f.id} value={f.id}>{f.name}</SelectItem>)}</SelectContent></Select></div>
            <div><Label>Rolle</Label><Select value={form.rolle} onValueChange={(v) => setForm({ ...form, rolle: v })}><SelectTrigger className="min-h-[48px]"><SelectValue /></SelectTrigger><SelectContent>{ROLLEN.map((r) => <SelectItem key={r} value={r}>{r}</SelectItem>)}</SelectContent></Select></div>
          </div>
          <div className="grid grid-cols-2 gap-3">
        ...
        <DialogFooter><Button variant="outline" onClick={onClose} className="min-h-[48px]">Abbrechen</Button><Button onClick={handleSave} disabled={!form.vorname.trim() || !form.nachname.trim() || saving} className="min-h-[48px] bg-accent hover:bg-accent-dark text-white">{saving ? 'Speichert...' : 'Speichern'}</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
}