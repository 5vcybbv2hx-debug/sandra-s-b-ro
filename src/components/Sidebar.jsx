  ...
  return (
    <aside className="hidden md:flex flex-col w-64 bg-white border-r border-border h-screen sticky top-0 shrink-0">
      ...
      <nav className="flex-1 px-3 space-y-1 overflow-y-auto">
        {items.map((item) => (<NavLink key={item.to} to={item.to} end={item.to === '/'} className={({ isActive }) => cn('flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-medium transition-colors min-h-[48px]', isActive ? 'bg-brand-light text-brand-dark' : 'text-muted-foreground hover:bg-cardbg hover:text-foreground')}><item.icon className="w-5 h-5 shrink-0" />{item.label}</NavLink>))}
      </nav>
