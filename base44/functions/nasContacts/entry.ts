...
Deno.serve(async (req) => {
  ...
  // HTTP client for self-signed certs (Deno)
  let client: any = null;
  try {
  ...
  <d:prop>
    ...
    <d:resourcetype />
        ...
        if (res.ok || res.status === 207) {
          ...
          const hrefMatches = text.match(/<d:href[^>]*>([^<]+)<\/d:href>/gi);
          if (hrefMatches) {
            for (const match of hrefMatches) {
              const href = match.replace(/<[^>]+>/g, '').trim();
              if (href.includes('addressbook') && !href.endsWith('/addressbooks/') && href.endsWith('/')) {
                return `${baseUrl}${href}`;
              }
            }
          }
...
<d:propfind xmlns:d="DAV:">
  <d:prop>
    ...
    const hrefMatches = text.match(/<d:href[^>]*>([^<]+\.vcf[^<]*)<\/d:href>/gi) || [];
    const vcfHrefs = hrefMatches.map(m => m.replace(/<[^>]+>/g, '').trim());
  ...
  async function pushContact(addressBookUrl: string, uid: string, vcard: string): Promise<string> {
    const vcfUrl = `${addressBookUrl}${uid}.vcf`;
  ...
  try {
    if (action === 'discover') {
      ...
      if (abUrl) {
        ...
        return Response.json({ success: false, error: 'Keine CardDAV Adresse gefunden. Ist Synology Contacts installiert?' }, { status: 404 });
      }
    }
    ...
    if (action === 'push') {
      ...
      const abUrl = await discoverAddressBook();
      if (!abUrl) {
        return Response.json({ success: false, error: 'CardDAV nicht gefunden. Synology Contacts installiert?' }, { status: 500 });
      }
